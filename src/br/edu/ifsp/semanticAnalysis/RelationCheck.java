...
public class RelationCheck implements RelationalQueryLanguageConstants {
	...
	String[] joinAttribute = new String[2];
	int joinAttributeCount = 0;
	...
	private void joinNodeCheck(JoinNode x, int scope, int[] binaryScopes) {
		...
		if (x.getLogicalSentenceNode() != null) {
			/*joinAttributeCount = 0;
			joinAttribute = new String[2];
			Set<String> r1Attributes = relation1.getAttributeNames();
			Set<String> r2Attributes = relation2.getAttributeNames();*/

			logicalSentenceNodeCheck(x.getLogicalSentenceNode(), scope);

			/*// Analyze the join operators
			if (r1Attributes.contains(joinAttribute[0]) && r2Attributes.contains(joinAttribute[1])
					&& !r1Attributes.contains(joinAttribute[1]) && !r2Attributes.contains(joinAttribute[0])) {
				
			} else if (r1Attributes.contains(joinAttribute[1]) && r2Attributes.contains(joinAttribute[0])
					&& !r1Attributes.contains(joinAttribute[0]) && !r2Attributes.contains(joinAttribute[1])) {

			}else{
				throwSemanticError("Ambiguous join operators");
			}*/
		} else {
			boolean valid = false;
		...
		if (x.getConditionalSentenceNode() != null)
			...
			if (x.getPosition().kind == RelationalQueryLanguageConstants.IDENTIFIER) {
				...
				if (!schema.getRelation("temporaryRelation" + (scope + 1)).hasAttribute(x.getPosition().image)) {
					...
					// Variables used for join operation
					if (joinAttributeCount < 2) {
						joinAttribute[joinAttributeCount] = x.getPosition().image;
					}
					joinAttributeCount++;
					// Set the attribute to the new relation
